"""Migrated beautiful_progress_widget from interest_rating.

Changes:
 - Adjusted import path for progress_styles to internal acfv.config.progress_styles.
 - Removed direct test main guard heavy usage (kept minimal if needed).
 - Kept Chinese labels and comments for continuity.
"""
from __future__ import annotations

import logging
from PyQt5.QtWidgets import (
    QWidget, QVBoxLayout, QHBoxLayout, QFrame, QProgressBar, QLabel, QScrollArea,
    QComboBox, QPushButton, QDialog
)
from PyQt5.QtCore import Qt, QTimer, QPropertyAnimation, QEasingCurve, QRect, QParallelAnimationGroup
from PyQt5.QtGui import QGraphicsOpacityEffect

from acfv.config.progress_styles import (
    MODERN_GRADIENT_STYLE, APPLE_STYLE, NEON_GLOW_STYLE, FLUENT_DESIGN_STYLE,
    GAMING_STYLE, MINIMAL_BW_STYLE, RAINBOW_STYLE, ADVANCED_STYLES, SUBSTAGE_INDICATOR_STYLES
)
...
class BeautifulProgressWidget(QWidget):
    ...
    def create_style_selector(self):  # noqa: D401
        """创建样式选择器（开发/调试用）"""
    ...
    def on_style_changed(self, style_name):  # noqa: D401
        """样式切换"""
        style_map = {
            "现代渐变": "modern",
            "苹果风格": "apple",
    ...
    def get_container_style(self):  # noqa: D401
        """根据主题返回容器样式"""
    ...
    def animate_style_change(self):  # noqa: D401
        """样式切换动画"""
    ...
    def toggle_animations(self):  # noqa: D401
        """切换动画效果"""
        enabled = self.animation_btn.isChecked()
        self.animation_btn.setText("✨ 动画效果" if enabled else "🚫 动画关闭")

    def set_progress_manager(self, progress_manager):  # noqa: D401
        """设置进度管理器"""
    ...
    def animate_show(self):  # noqa: D401
        """显示动画"""
        if hasattr(self, 'animation_btn') and self.animation_btn.isChecked():
    ...
    def animate_hide(self):  # noqa: D401
        """隐藏动画"""
        if hasattr(self, 'animation_btn') and self.animation_btn.isChecked():
            def hide_widget():  # noqa: D401
                self.setVisible(False)
    ...
    def update_display(self):  # noqa: D401
        """更新显示内容"""
    ...
    def get_stage_icon(self, stage_name):  # noqa: D401
        """根据阶段名称返回对应图标"""
        icon_map = {
            "音频提取": "🎵",
            "说话人分离": "👥",
    ...
    def animate_progress_update(self, start_value, end_value):  # noqa: D401
        """进度条数值更新动画"""
        if hasattr(self, 'animation_btn') and self.animation_btn.isChecked():
    ...
    def update_substage_display(self, stage_details):  # noqa: D401
        """更新子阶段显示"""
    ...
    def create_substage_indicator(self, name: str, status: str):  # noqa: D401
        """创建子阶段指示器"""
    ...
    def update_stats_display(self, progress, stage_details):  # noqa: D401
        """更新统计信息"""
        try:
            completed_stages = sum(
                1 for i in range(len(stage_details.get("substages", [])))
...
class ProgressStylePreview(QDialog):
    ...
    def create_style_preview(self, name, theme):  # noqa: D401
        container = QFrame()
...
class SimpleBeautifulProgressBar(QWidget):
    ...
    def update_display(self):  # noqa: D401
        ...
        try:
            ...
            stage_text = (
                current_progress.get('current_stage')
    ...
    def setValue(self, value):  # noqa: D401
        self.progress_bar.setValue(value)

    def setRange(self, minimum, maximum):  # noqa: D401
        self.progress_bar.setRange(minimum, maximum)

    def setStatus(self, text):  # noqa: D401
        self.status_label.setText(text)

    def setProgress(self, value, status=None):  # noqa: D401
        self.setValue(value)
        if status:
            self.setStatus(status)

    def show_progress(self, show=True):  # noqa: D401
        self.setVisible(show)

    def hide_progress(self):  # noqa: D401
        try:
            self.setVisible(False)
    ...
    def start_progress(self, title: str = "处理中..."):  # noqa: D401
        try:
            self.progress_bar.setValue(0)
    ...
    def update_status(self, status: str, detail: str | None = None):  # noqa: D401
        try:
            if detail:
    ...
    def update_progress(self, value: float | int, status: str | None = None, detail: str | None = None):  # noqa: D401
        try:
