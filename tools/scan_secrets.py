...
PATTERNS = [
    ...
    re.compile(r'(?i)(oauth[_-]?token|access[_-]?token|client[_-]?secret)\s*[:=]\s*["\']?[0-9A-Za-z._\-]{16,}'),
...
def iter_files(root: pathlib.Path):
    for p in root.rglob('*'):
        ...
        if any(str(p).endswith(suf) for suf in IGNORE_SUFFIXES):
            continue
        if p.suffix in {'.py', '.txt', '.json', '.env', '.yaml', '.yml'} and not p.name.endswith('.example'):
            yield p
...
def main():
    ...
    if flagged:
        ...
        for f, val in flagged:
            print(f'  - {f}: {val[:50]}')
        print('\nBlock commit. Please remove or move secrets.')
        return 1
